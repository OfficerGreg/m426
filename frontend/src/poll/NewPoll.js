...
class NewPoll extends Component {
    ...
    handleSubmit(event) {
        ...
        .then(response => {
            ...
            if(error.status === 401) {
                this.props.handleLogout('/login', 'error', 'You have been logged out. Please login to create a Project.');    
            } else {
                notification.error({
                    message: 'BingChilling',
    ...
    validateChoice = (choiceText) => {
        if(choiceText.length === 0) {
            return {
                validateStatus: 'error',
                errorMsg: 'Please enter a member'
            }
                    ...
                    <Form onSubmit={this.handleSubmit} className="create-poll-form">
                        ...
                        <FormItem className="poll-form-row">
                            <Button type="primary" 
                                ...
                                className="create-poll-form-button">Create Project</Button>
                        </FormItem>
                    </Form>
                </div>    
            </div>
